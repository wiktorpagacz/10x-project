 ...
 */
export function CharacterCounter({ current, max, min, threshold = 70, label }: CharacterCounterProps) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      {min !== undefined && current < min && (
        <span className={cn("text-xs", colorClass)}>(min: {min.toLocaleString()})</span>
      )}
    </div>
  );
}
