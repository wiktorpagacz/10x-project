...
type CreateFlashcardsBatchRequest = z.infer<typeof createFlashcardsBatchSchema>;
...
export async function POST(context: APIContext): Promise<Response> {
  try {
    ...
    const { generation_id: generationId, flashcards } = validationResult.data as CreateFlashcardsBatchRequest;
    ...
    if (generationError) {
      throw new Error(`Database query failed when verifying generation: ${generationError.message}`);
    }
    ...
    const createdFlashcards: FlashcardDto[] = await createBatch(supabase, userId, generationId, flashcards);
