...
- Return ONLY the JSON object, no markdown code blocks, no additional text`;
  ...
  private validateRequest(request: ChatCompletionRequest): void {
    ...
    for (let i = 0; i < request.messages.length; i++) {
      ...
      if (!message.role || !message.content) {
        throw new OpenRouterError(
          400,
          'INVALID_REQUEST',
          `Message at index ${i} must have both role and content`
        );
      }
    ...
    if (request.presencePenalty !== undefined) {
      if (
        typeof request.presencePenalty !== 'number' ||
        request.presencePenalty < -2 ||
        request.presencePenalty > 2
      ) {
        throw new OpenRouterError(
          400,
  ...
  private parseMarkdownFlashcards(content: string): GeneratedFlashcard[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const frontPatterns = [
        /^(?:\*\s*)?(?:Front|Question|Q):\s*(.+)$/i,
        /^\d+\.\s*(?:Front|Question|Q):\s*(.+)$/i,
      ];
